...
export function processBattingStats(player: any): ProcessedStats {
  ...
  const points = calculateBattingPoints(stats);
  ...
  return {
    points,
    battingStats,
    pitchingStats: PitchingStats.create(0, 0, 0, 0, 0, 0, 0, 0, null, 0),
  };
}

export function processPitchingStats(player: any): ProcessedStats {
  const stats: MLBStats = {
    inningsPitched: player.stats?.pitching?.inningsPitched ?? 0,
    earnedRuns: player.stats?.pitching?.earnedRuns ?? 0,
    pitchingStrikeouts: player.stats?.pitching?.strikeOuts ?? 0,
  ...
  return {
    points,
    battingStats: BattingStats.create(0, 0, 0, 0, 0, 0, 0, 0),
    pitchingStats,
  };
}
